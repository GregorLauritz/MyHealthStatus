...
- **Directional Delta Indicators with Baseline Context:** Dynamic biometric comparisons (resting heart rate and nocturnal HRV) compute directional delta indicators (`↑ X`, `↓ Y`, or `0`) relative to 30-day baseline context within `WidgetSnapshotMapper`. In expanded contexts, this is rendered with explicit baseline attribution (`"↑ 3 vs baseline"`), while compact pill contexts render the concise signed arrow indicator (`StatusPill` / `MetricPill`) for glanceable interpretation.
