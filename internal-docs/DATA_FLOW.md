...
- **Localized Category Representation:** Readiness categories in the snapshot are formatted to localized strings (`widget_status_optimal`, `widget_status_good`, `widget_status_fair`, `widget_status_low`) dynamically via `ReadinessStatusFormatter` at Glance render time.
