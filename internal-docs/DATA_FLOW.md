...
└──────────────┬─────────────────────────────────────────────────────────────┘
               ...
               ▼
┌──────────────────────────────┐
│  RoomTransactionRunner       │   parent txn + 500-row HR/HRV transactions
└──────────────┬───────────────┘
...
| `StepAttribution`             | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/StepAttribution.kt`                             | Canonical step-attribution rule shared across both sync flows (R2-HC-004, OD-2): resolves `(day, stepsMap, stepsDeviceSelected, recomputeOnly)` to `Long?`. Recompute-only passes resolve to `null` (preserve stored); selected device with missing data resolves to `0L` (resync authority corrects stale data); non-device or fetched data resolves to `stepsMap[day]`. |
...
| `PeriodicHealthSyncWorker`    | `app/src/main/kotlin/app/readylytics/health/workers/PeriodicHealthSyncWorker.kt`        | "Background Sync" toggle in Settings. Before resolving its lazy Room-backed `HealthSyncUseCase` or lazy `ForegroundSyncController`, it requires `DatabaseReadiness.Ready`; otherwise it retries without opening Room or constructing the controller graph. Once ready, this `@HiltWorker` periodic **standard (non-foreground) worker** calls `HealthSyncUseCase.sync(windowDays = 2)` (shares `syncMutex` with the other two flows), bridges progress to `ForegroundSyncController`, triggers `WidgetUpdatePort.updateAllWidgets()` upon successful sync, shows/dismisses a silent transient notification (`SyncNotifications.BACKGROUND_SYNC_CHANNEL_ID`) via `NotificationManagerCompat` directly — no `setForeground()`, since `READ_HEALTH_DATA_IN_BACKGROUND` already permits background HC reads and starting a foreground service from a periodic background worker risks `ForegroundServiceStartNotAllowedException` on API 34+. Ordinary failures return `Result.retry()`; `REQUIRES_HISTORICAL_RESYNC` enqueues `WorkerScheduler.scheduleResyncWorker()` and finishes successfully so durable catch-up runs once without periodic retry churn. |
...
so re-ingestion is idempotent, but entity construction itself happens one layer down across the shared persistence mappers (`core/database/src/main/kotlin/app/readylytics/health/core/database/data/local/HealthIngestionInputMappers.kt`, `HealthIngestionVitalsMappers.kt`, and `HealthIngestionSessionSpanMappers.kt`). Both bulk/resync and incremental Changes API ingestion paths share this unified mapper set and persistence boundary.
...
### 1.4 Room storage — `HealthDatabase` (`@Database(version = 17)`)
...
| `RasCalculator`                    | `core/scoring/src/main/kotlin/app/readylytics/health/core/scoring/domain/scoring/RasCalculator.kt`                    | `calculateDailyTrimp(..., trimpModel = TrimpModel.BANISTER)` switches per model — **BANISTER is the operational default** (default parameter value). Applies the TRIMP dead zone (`ScoringConstants.Trimp.MIN_HR_ABOVE_RHR_BPM` = 5 bpm above RHR baseline). `calculateDailyRas()` converts TRIMP → RAS via a profile scaling factor (capped at 75). |
...
`SettingsDefaults.CURRENT_SCORING_VERSION` to **3**. Training Readiness raises the current scoring
version to **4**: startup enqueues the ordinary full retained recompute-only resync
(`WorkerScheduler.scheduleResyncWorker(recomputeOnly = true)`) whenever `storedScoringVersion < CURRENT_SCORING_VERSION`,
so its normal oldest-first chronology and residual-fatigue accumulator populate the default Training
Readiness projections for existing v17 rows. This is deliberately not the parameter-only projection
branch: v16 rows have no retained projection to update. `HealthResyncWorker.persistPostRecomputeState()`
...
**PERF — batched wearable VO2 Max lookups in the walk-forward.** `FinalSummaryAssembler.resolveVo2Max`
resolves the wearable-reported VO2 Max component (`Vo2MaxSourceResolver`'s `AUTO`/`WEARABLE_ONLY`
inputs) by finding the most recent `Vo2MaxRecordEntity` within a 30-day trailing lookback of the
target day. A multi-day walk-forward fetches `Vo2MaxRecordDao.getByTimeRange` **once** for the whole
range (`ScoringRepositoryImpl.fetchWalkForwardVo2MaxContext`, extended 30 days back) into a
`WalkForwardVo2MaxContext` (`TreeMap<Long, Float>` keyed by `timestampMs`), instead of every
recomputed day issuing its own `Vo2MaxRecordDao.getLatestInWindow` point query; each day looks up
`floorEntry(nextDayMidnightMs)` and checks that entry's timestamp against its own 30-day lookback
floor — reproducing the exact bounded-window semantics the per-day query used. The single-day
overload (context null) still queries per day.
...
| `WalkForwardTrimpContext` / `WalkForwardBaselineContext` / `WalkForwardFatigueContext` / `WalkForwardVo2MaxContext` | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/repository/` | PERF-002/WP-20/WP-22. Held by a multi-day walk-forward (`ResyncRangeUseCase`'s RECOMPUTE phase) across every recomputed day: the TRIMP series (two `TreeMap<LocalDate, Float>`, fetched once via `ScoringRepository.fetchWalkForwardTrimpContext`), the RHR/HRV baseline sleep-session superset (fetched once via `fetchWalkForwardBaselineContext`, delegating to `BaselineComputer.prefetchWalkForwardSessions`), — WP-27, see §2.8 — the residual-fatigue workout-impulse series + running accumulator (`fetchWalkForwardFatigueContext`), and the wearable VO2 Max readings (`TreeMap<Long, Float>` keyed by `timestampMs`, fetched once via `fetchWalkForwardVo2MaxContext`, see §2.6's VO2 Max paragraph above). |
| `ScoringRepositoryImpl.computeDailySummary` | `core/database/src/main/kotlin/app/readylytics/health/core/database/data/repository/ScoringRepositoryImpl.kt` | Fetches the day's SQL-bucketed HR rows (`HeartRateDao.getMinuteBuckets`) plus sleep/workout intervals, feeds them to `AssembleEverydayLoadInputUseCase`, then `BuildLoadSeriesUseCase` for the ATL/CTL/strain/load assembly, and persists both `*WorkoutOnly` and `*EverydayHr` variants (TRIMP, RAS, total RAS, ATL, CTL, Strain Ratio, Load Score, Readiness, Training Load Readiness, Training Readiness) plus shared Residual Fatigue/acute recovery and `everydayCoverageMinutes`/`everydayLoadConfidence` on `DailySummaryEntity`. Accepts one `WalkForwardContexts` holder carrying the optional `WalkForwardTrimpContext`/`WalkForwardBaselineContext`/`WalkForwardFatigueContext`/`WalkForwardVo2MaxContext` (single `computeAndPersistDailySummary` signature, §2.6/§2.8) to slice shared in-memory data instead of re-querying per day. `ComputeResidualFatigueUseCase` and `ComputeTrainingReadinessUseCase` are injected like every other scoring use case (grouped with them in `ScoringDayUseCases`), never hand-constructed. |
| `LoadSourceSelector` | `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/model/LoadSourceSelector.kt` | Pure mode-aware projection: `select*` functions pick the `*WorkoutOnly`/`*EverydayHr` variant column or derived value (TRIMP, ATL, CTL, strain ratio, RAS, legacy Readiness, Training Readiness, earliest data date) matching `strainLoadSourceMode`/`rasSourceMode`. Zero Android dependencies. |
...
value is a raw, unbounded, non-normalized, provisional, workout-only derived input. It feeds Training
Readiness when available but does not modify legacy Readiness or Load Score; those remain separate
projections. Everyday-HR load is never a Residual Fatigue impulse source.
...
Every persisted snapshot is therefore an *end-of-day* value. For the current day the persistence evaluation
point is in the future, so today's stored row is a projection to the end of today, not fatigue at the moment
of the sync. This keeps the stored series deterministic and wall-clock independent; the snapshot evaluation
point must not be capped at `now`. The optional current-day Dashboard card instead calls the non-persisting
`computeLive(nowMs, prefs)` path described below, and never derives a live value from the stored snapshot.
...
**Training Readiness projection integration.** The pure formula source is
`ComputeTrainingReadinessUseCase` (`core/scoring/.../domain/scoring/`); formula details and parameter
ranges belong in ABOUT.md and the in-app About copy, not here. In an ordinary daily, full-resync, or
recompute-only scoring walk, `FinalSummaryAssembler` first obtains the legacy scored summary and stamps
the Residual Fatigue snapshot. It then calls the pure projection once for the workout-only Load Score and
once for the everyday-HR Load Score, using one immutable preference snapshot and
`UserPreferences.appliedTrainingReadinessConfig()`. The normal path therefore adds no Health Connect or
raw-data query and calculates workout TRIMP only once. It persists five nullable `daily_summaries` columns:
shared `acuteLoadRecovery`; `trainingLoadReadinessWorkoutOnly` and
`trainingLoadReadinessEverydayHr`; and `trainingReadinessWorkoutOnly` and
`trainingReadinessEverydayHr`. `LoadSourceSelector.selectTrainingReadiness` projects the branch matching
the active Strain / Training Load source for the default-hidden Dashboard card. Legacy Readiness columns
and every Advisor/AI consumer remain unchanged; the Advisor continues selecting legacy Readiness.

**Parameter-only durable branch.** Editing the S/w controls writes only the editable DataStore fields, so
pending values cannot leak into ordinary sync/recompute output. The explicit **Recalculate Training
Readiness** action captures an immutable `TrainingReadinessConfig` in WorkManager input and calls
`WorkerScheduler.scheduleTrainingReadinessRecompute`. It appends the same `HealthResyncWorker` unique
`RESYNC_WORK_NAME` chain (`MODE_TRAINING_READINESS`) and reuses `WorkInfo`/`RecalcProgress`, foreground
notification, exponential backoff, and `HealthSyncUseCase.syncMutex`. The worker delegates to
`FullHistoricalResyncUseCase.executeTrainingReadinessProjection`, which retention-bounds the range and
then calls `TrainingReadinessProjectionRecomputeUseCase`. That use case performs exactly one retained
summary query, maps both source projections in memory, and batch-upserts once inside one Room transaction.
It performs no Health Connect access, raw-sample DAO read, TRIMP/load-EMA calculation, or residual-fatigue
reconstruction. Progress callbacks do not write per day, so Compose receives one Room invalidation after
commit rather than a per-day invalidation loop.

A killed/cancelled/failed projection transaction rolls back, preserving the prior five score values and
the prior applied S/w pair. Only after the batch commit succeeds does the worker advance the applied pair;
retrying the same captured input is idempotent. If the user edits again while an older request runs, those
new editable values remain pending after the older pair succeeds. Normal daily/full recomputes continue to
read only the applied pair throughout.
...
- **Resolver:** `FinalSummaryAssembler` selects the estimated-value calculator from the user's `vo2MaxEstimationMethod` preference (`Vo2MaxEstimationMethod.HR_RATIO` | `MATERKO_ADAPTED`), then `Vo2MaxSourceResolver.resolve` chooses between that tagged estimate and the latest wearable value according to `vo2MaxSourceMode` (Auto, Wearable only, Estimate only). Both estimates are unavailable until scoring calibration completes; the Materko-adapted path consumes the same day's stable resting-HR baseline plus `exp(hrvMuMssd)` (the resolved frozen-or-computed RMSSD baseline used by the daily summary).
- **Estimation:** `UthVo2MaxCalculator` implements the Uth et al. (2004) resting-HR-ratio method. `MaterkoAdaptedVo2MaxCalculator` implements the experimental resting-RHR + RMSSD-baseline adaptation and documents its departures from the published Materko model (CDR omitted, pNN50 approximated from RMSSD) and its supported-input guards — see the calculator's own doc comment for the full derivation. Persisted sources distinguish `ESTIMATED_UTH` from `ESTIMATED_MATERKO_ADAPTED`; there is no Room schema change for this addition, and existing `ESTIMATED_UTH` rows are unaffected. The two methods diverge on out-of-range behavior: `UthVo2MaxCalculator` clamps its result to `[15.0, 95.0]`, while `MaterkoAdaptedVo2MaxCalculator` returns `null` (no estimate) for results outside its supported range rather than clamping.
...
`observeByTimeRange`, so a resync's 500-row ingest batches invalidating the Flow re-run a
...
| :------------------------------------------------------------------------- | :-------------------------------------------------- | :--------------------------------------------------------------------------------------- |
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/repository/HealthConnectPermissionChecker.kt`                      | UI / Feature — narrow permission probe contract     | `has*Permission()` probes for feature gates/cards                                        |
...
| `core/model/src/main/kotlin/app/readylytics/health/core/model/domain/sync/StepAttribution.kt`                                              | Ingestion — shared attribution rule                 | `resolve()`: one rule for daily sync and resync (R2-HC-004)                              |
...
| `core/scoring/src/main/kotlin/app/readylytics/health/core/scoring/domain/scoring/ComputeTrainingReadinessUseCase.kt` | Processing — Training Readiness (pure) | converts Residual Fatigue to acute recovery, blends the source-matched Load Score, and delegates final scoring to the existing Readiness calculator (§2.8); ABOUT.md owns formula details |
...
| `core/database/src/main/kotlin/app/readylytics/health/core/database/data/repository/FinalSummaryAssembler.kt` | Processing — summary assembly | stamps `residualFatigue`, then computes/stores both source-specific Training Readiness projections with the applied S/w pair (§2.8) |
...
| `feature/widget/src/main/kotlin/app/readylytics/health/feature/widget/ui/WidgetDeltaFormatter.kt`                        | UI / Widget — delta & secondary metric formatter    | Formats raw integer deltas and secondary recovery metrics via localized string resources |
| `feature/widget/src/main/kotlin/app/readylytics/health/feature/widget/ui/ReadinessStatusFormatter.kt`                    | UI / Widget — status string formatter               | Resolves localized string resources for readiness tiers and calibration states          |
| `feature/widget/src/main/kotlin/app/readylytics/health/feature/widget/receiver/BaseGlanceWidgetReceiver.kt`              | UI / Widget — base broadcast receiver               | Abstract Glance receiver dispatching onUpdate/onEnabled to WidgetUpdatePort via goAsync |
| `feature/widget/src/main/kotlin/app/readylytics/health/feature/widget/di/WidgetReceiverEntryPoint.kt`                     | UI / Widget — Hilt entry point                      | Provides WidgetUpdatePort to BroadcastReceivers outside standard constructor injection  |
...
1. **2x2 Recovery Glance (`RecoveryGlanceWidget`):** M3 single-surface card featuring large readiness gauge/score, localized tier StatusPill, secondary recovery metric (HRV/RHR) with directional delta context, and sleep score/duration with dual deep-link click targets (Dashboard and Sleep).
2. **4x2 Daily Strain & Recovery (`StrainRecoveryWidget`):** 3-column split layout separated by vertical subtle dividers featuring Readiness gauge and tier/calibration StatusPill, Sleep metric with duration and Deep/REM architecture percentages, and Strain column with target range and step count with progress bar, deep-linking into Dashboard, Sleep, and Workouts tabs.
3. **4x1 Minimal Vitals Strip (`VitalsStripWidget`):** Horizontal multi-column single-surface strip displaying resting heart rate with directional delta pill, nocturnal HRV lnRMSSD, sleep duration, and daily steps with progress bar, separated by subtle dividers, deep-linking to Vitals/Dashboard.
...
Update Triggers:
  ├── App Startup (DatabaseReadyStartupInitializer upon DatabaseReadiness.Ready)
  ├── App Foreground (SyncViewModel.onAppForeground)
  ├── Foreground Sync Completion (SyncViewModel via syncCompletedEvent)
  ├── Periodic Background Sync (PeriodicHealthSyncWorker)
  ├── Full Historical Resync (HealthResyncWorker on success)
  └── Widget Placement / Broadcast (BaseGlanceWidgetReceiver onUpdate / onEnabled via goAsync)
  │
  ▼ triggers
...
WidgetSnapshot (pure Kotlin data structure serialized to JSON)
  │
  ▼ unconditionally persists via WidgetSnapshotDefinition.getDataStore(...)
Glance DataStore ("readylytics_widget_snapshot.json" via WidgetSnapshotSerializer)
  │
  ▼ GlanceAppWidgetManager.update(context, glanceId) for each active GlanceId
...
- **Unconditional Snapshot Persistence:** In `WidgetUpdateCoordinator.updateAllWidgets()`, `persistSnapshot(snapshot)` is executed unconditionally before iterating over active glance IDs. This guarantees that `readylytics_widget_snapshot.json` is created and populated in DataStore even when zero widgets are currently placed on the launcher, preventing empty/placeholder states when the user places their first widget.
- **Self-Healing Receiver Lifecycle (`BaseGlanceWidgetReceiver`):** `RecoveryGlanceWidgetReceiver`, `StrainRecoveryWidgetReceiver`, and `VitalsStripWidgetReceiver` extend `BaseGlanceWidgetReceiver`. Its `onUpdate` and `onEnabled` handlers enter `goAsync()` and resolve `WidgetUpdatePort` via `WidgetReceiverEntryPoint` (Hilt `@EntryPoint`), triggering immediate snapshot refresh and widget updates when widgets are newly added or refreshed by launcher broadcasts.
- **Multi-Point Push Updates via `WidgetUpdateCoordinator`:** Updates are pushed to Glance widgets across key lifecycle moments: app startup (`DatabaseReadyStartupInitializer`), app foreground resume (`SyncViewModel.onAppForeground()`), foreground sync completion (`SyncViewModel`), background periodic sync (`PeriodicHealthSyncWorker`), historical resync (`HealthResyncWorker`), and widget placement (`BaseGlanceWidgetReceiver`). The coordinator builds a fresh snapshot for the current date in the user's active scoring timezone (`prefs.scoringZone()`) and pushes it across all active glance IDs before requesting widget recomposition. Updating is error-isolated across widget groups with `safeUpdateWidgetGroup` (wrapping each in `runCatching` while rethrowing `CancellationException`), ensuring a failure in one widget group does not abort updating others.
- **Material 3 Single-Surface Architecture:** Rather than floating nested cards, widgets render inside a unified `WidgetSurface` using `GlanceTheme.colors.widgetBackground` with standard 16.dp corner rounding (`cornerRadius(16.dp)`). Structural columns and metric cells are cleanly demarcated via `SubtleDivider` (1.dp line tinted with `GlanceTheme.colors.outline`). Circular `IconBadge` components (`size = 28.dp`, `secondaryContainer` fill, 16.dp vector drawable tinted with `onSecondaryContainer`) provide a unified iconography language across metric cells, while `StatusPill` renders localized category/calibration chips using `secondaryContainer` / `onSecondaryContainer`.
- **Responsive Size Breakpoints & Density-Based Collapsing:** All three widgets declare `override val sizeMode = SizeMode.Responsive(...)` with tailored breakpoint sets:
  - `RecoveryGlanceWidget`: Compact (110x110 dp) and Standard (140x140 dp). In Compact mode, auxiliary metric rows collapse to preserve primary readiness score and status pill legibility.
  - `StrainRecoveryWidget`: Compact (250x110 dp) and Standard (280x120 dp). Under tighter heights or widths, secondary labels and sub-metrics gracefully collapse while maintaining clear 3-column readiness, sleep, and strain summaries.
  - `VitalsStripWidget`: Compact (200x40 dp) and Standard (260x60 dp). Adapts between dense one-row layout and spacious multi-cell horizontal strip, scaling icon badge padding and subtext based on available launcher cell dimensions.
- **Directional Delta Indicators with Baseline Context:** Dynamic biometric comparisons (resting heart rate and nocturnal HRV) compute raw integer deltas (`rhrDelta` and `hrvDelta`) relative to baseline context within `WidgetSnapshotMapper`. At Glance render time, `WidgetDeltaFormatter` formats these using localized string resources (`widget_delta_up_format`, `widget_delta_down_format`, `widget_delta_vs_baseline_format`, `widget_secondary_hrv_format`, `widget_secondary_rhr_format`) with directional indicators (`↑ X`, `↓ Y`, or `0`), baseline attribution, and secondary metric line formatting.
- **Localized Category Representation:** Readiness categories in the snapshot (aligned with the app's Readiness tiers: Peak >= 85, Maintain >= 60, Caution >= 30, High Fatigue < 30) are formatted to localized strings (`widget_status_peak`, `widget_status_maintain`, `widget_status_caution`, `widget_status_high_fatigue`, with backward-compatible fallbacks) dynamically via `ReadinessStatusFormatter` at Glance render time.
